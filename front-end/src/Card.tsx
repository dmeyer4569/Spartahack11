import type { PantryItem } from './types/item'

interface CardProps {
  item: PantryItem;
  locationName?: string;
}

function Card({item, locationName}: CardProps) {
  const handleRemove = async (e: React.MouseEvent) => {
    e.stopPropagation();

    if(!confirm(`Are you sure you want to remove ${item.name}?`)) return;

    try {
      const response = await fetch(`http://10.249.231.63:8001/api/remove/${item.id}`, {
        method: 'DELETE',
        headers: {
          'accept': 'application/json'
        }
      });

      if (response.ok) {
        window.location.reload();
      } else {
        console.error('Failed to remove item');
      }
    } catch (error) {
      console.error('Error removing item:', error);
    }
  }
  return (
    <div className="group rounded-2xl bg-white shadow-sm border border-gray-100 overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
      <div className="h-48 w-full overflow-hidden relative">
        <img 
          src={`http://10.249.231.63:8001/uploads/images/${item.img_path}`}
          alt={item.name}
          className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-100"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />

        <button
          onClick={handleRemove}
          className="absolute top-3 left-3 z-10 bg-red-500 hover:bg-red-600 text-white w-7 h-7 rounded-full shadow-lg flex items-center justify-center transition-colors duration-200"
          title="Remove Item"
        >
          <span className="text-lg leading-none">&times;</span>
        </button>
        
          
        <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg shadow-sm flex items-center justify-center">
          <span className="text-[12px] font-bold text-blue-500 leading-none">
           {item.quantity}
          </span>
        </div>
      </div>

      <div className="p-5">
        <div className="flex items-center justify-between gap-2 mb-1">
          <h3 className="text-lg font-bold text-gray-900 capitalize truncate">
            {item.name}
          </h3>

          {locationName && (
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">
              {locationName}
            </span>
          )}
        </div>
        
        <div className="flex items-center justify-between mt-4">
          <span className="text-xs font-semibold text-gray-400 uppercase">Expires</span>
          <span className="text-sm font-bold text-red-500 bg-red-50 px-3 py-1 rounded-full">
            {item.expire}
          </span>
        </div>
      </div>
    </div>
  )
}

export default Card
